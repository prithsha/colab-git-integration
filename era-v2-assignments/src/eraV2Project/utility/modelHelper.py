...
def print_analysis_shapes(batch_data, batch_labels, batch_predictions):
        ...
        logger.debug(f" Shape of data : {batch_data.shape}, label: {batch_labels.shape}")
        logger.debug(f"Shape of predicted data : {batch_predictions.shape}")
        logger.debug(f"labels : {batch_labels}")
        logger.debug("-----------------------------------------------------")
        logger.debug(f"argmax_data : {argmax_data}")

        logger.debug(f"true_indices : {true_indices}")
        logger.debug(f"false_indices : {false_indices}")
...
def get_correct_prediction_count(predictions : torch.Tensor, valid_labels):
    ...
    result = predictions.argmax(dim=1).eq(valid_labels).sum().item()
